### this is img encoder code from pixel Nerf
"""
...
class ConvEncoder(nn.Module):
    ...
    def forward(self, x):
        x = encoder_util.same_pad_conv2d(x, padding_type=self.padding_type, layer=self.conv_in)
        x = self.conv_in(x)

        inters = []
        for i in range(0, self.n_down_layers):
            conv_i = getattr(self, "conv" + str(i))
            x = encoder_util.same_pad_conv2d(x, padding_type=self.padding_type, layer=conv_i)
            x = conv_i(x)
            inters.append(x)

        x = encoder_util.same_pad_conv2d(x, padding_type=self.padding_type, layer=self.conv_mid)
        x = self.conv_mid(x)
        ...
        for i in reversed(range(0, self.n_down_layers)):
            ...
            x = deconv_i(x)
            x = encoder_util.same_unpad_deconv2d(x, layer=deconv_i)
        x = self.deconv_last(x)
        x = encoder_util.same_unpad_deconv2d(x, layer=self.deconv_last)
        return x
