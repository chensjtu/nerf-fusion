...
class ScannetSequence(RGBDSequence):
    def __init__(self, path: str, args: argparse.Namespace, start_frame: int = 0, end_frame: int = -1, 
                            first_tq: list = None, load_gt: bool = False, device = None):
        super().__init__()
        self.args = args
        self.path = Path(path)
        self.device = device
        ...
        self.map = Map_for_preprocessing(self.args, device=device)
    ...
    def __next__(self):
        ...
        depth_img_path = self.path / self.depth_names[self.frame_id]
        rgb_img_path = self.path / self.color_names[self.frame_id]
        # pose_path = self.path / self.pose_names[self.frame_id]
    ...
    def forward(self, index):
        self.frame_id = index
        cur_frame = self.__next__()
        corner_pos, valid_mask = self.preprocess(cur_frame)


        return corner_pos

    def preprocess(self, frame):
        ### conducting voxelization
        ...
        # frame_pose = self.tracker.track_camera(frame.rgb, frame.depth, frame.calib,
        #                 self.first_iso if len(self.tracker.all_pd_pose) == 0 else None, for_pc=True)
        # tracker_pc, tracker_normal = self.tracker.last_processed_pc
        ...
        valid_mask = motion_util.remove_out_uv(uv, self.w,self.h)
        return corner_pos, valid_mask

    def reprojection(self, Pw, frame):
        K = torch.from_numpy(frame.calib.to_K()).to(self.device).float() # 3*3
        pose = torch.from_numpy(frame.gt_pose.matrix[:3,:]).to(self.device).float() #3*4
...
class Map_for_preprocessing:
    def __init__(self, args: argparse.Namespace, device="cpu"):
        ...
        self.n_xyz = np.ceil((np.asarray(args.mapping.bound_max) - np.asarray(args.mapping.bound_min)) / args.mapping.voxel_size).astype(int).tolist()
        ...
        for p in self.cold_vars.keys():
            setattr(Map_for_preprocessing, p, property(
                fget=functools.partial(Map_for_preprocessing._get_var, name=p),
                fset=functools.partial(Map_for_preprocessing._set_var, name=p)
            ))
    ...
    def integrate_keyframe(self, surface_xyz: torch.Tensor, surface_normal: torch.Tensor, do_optimize: bool = False, async_optimize: bool = False):
        ...
        # logging.info(f"there are {surface_grid_id[0], surface_grid_id[1],surface_grid_id[2]} voxels in total")
        surface_grid_id = self._linearize_id(surface_grid_id) # 39679
        ...
        if invalid_surface_ind.sum() > 0:
            ## this means there do exist valid points
            invalid_flatten_id = torch.unique(surface_grid_id[invalid_surface_ind]) # after romove sparse, voxel num is 1246
        ...
        if encoder_voxel_pos.size(0) > 0:
            ...
            voxel_index_fla = torch.unique(gatherd_surf_gid) # 1072
            voxel_index = self._unlinearize_id(voxel_index_fla)
        ...
        # # End if do_optimize
        pc_pos = self.get_extract_pos(voxel_index)
