...
class NerfusionRenderer(nn.Module):
    ...
    def forward(self, rays_o: Tensor, rays_d: Tensor, vox_idx: Tensor, 
        t_near: Tensor, t_far: Tensor, ray_hits: Tensor, imgs: Tensor, KRcam: Tensor):
        '''
        Args:
            ...
            hits: [N_rays]
        '''
        # first, enc imgs to generate feats
        feats = self.voxel_embedder(imgs) # N 64 240 320
        # backproject
        self.vox_rep.back_project(feats, KRcam)

        n_rays = ray_hits.shape[0]
        if n_rays > self.chunk: # iter the data
    ...
    def step_forward(self, pts: Tensor, p2v_idx: Tensor, rays_d: Tensor, 
        ...
        vox_embeds = self.vox_rep.get_feats(pts, p2v_idx, self.vox_rep) #[n_pts, feat_c]
        nn_out = self.model(
    ...
    def extract_mesh(self):
        pass
        # sparse_pts = self.vox_rep.get_pts()