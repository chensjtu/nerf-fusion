...
class NerfusionRendererTest(nn.Module):
    ...
    @torch.no_grad()
    def extract_mesh(self,pt_per_edge=4,threshold=0,export_mesh=False, location=None):
        sparse_pts,p2v_idx,occ= self.vox_rep.get_pts(pt_per_edge) # N_voxels, pt_per_vox, 3 / N_voxels
        sh = list(occ.shape)
        num_voxel, pt_per_vox= sparse_pts.shape[:2]
        if num_voxel > self.ext_chunk:
            ret = [self.prd_sigma(sparse_pts[ci:ci+self.ext_chunk], p2v_idx[ci:ci+self.ext_chunk])
            for ci in range(0, num_voxel, self.ext_chunk)]
            ret=torch.cat(ret,dim=0).cpu() # N_voxels, pt_per_vox
        else:
        ...
        vertices, triangles = marching_cubes(sigma.numpy(), threshold)
    ...
    def prd_sigma(self,pts,p2v_idx):
        ...
        sigma = F.relu(nn_out['sigma'])
        return sigma.reshape(n_vox,pt_per_vox)
