...
def main():
    ...
    # corner_pos = seq_dataset.forward(index = 0)
    # correspond_pose = seq_dataset.gt_trajectory[seq_dataset.frame_id].matrix
    ...
    for epoch in epoch_bar:
        ...
        for rgbd_data in lif_loader:
            ...
            K = rgbd_data["K"].to(device)
            depth = rgbd_data["depth"].to(device)
            rgb = rgbd_data["rgb"].to(device)
            gt_pose = rgbd_data["gt_pose"].to(device)
            # note that the pose is c2w, if w2c, use inv
